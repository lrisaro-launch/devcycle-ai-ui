...
const InfrastructureConfiguration: React.FC = () => {
    ...
    const handleArrayStringChange = (section: string[], label: string, idx: number, value: string) => {
        setForm((prev: any) => {
            const updated = { ...prev };
            let ref: any = updated;
            for (const s of section) ref = ref[s];
    ...
    const renderField = (label: string, value: any, sectionPath: string[], idx?: number) => {
        const key = idx !== undefined ? `${sectionPath.join(".")}.${idx}.${label}` : `${sectionPath.join(".")}.${label}`;
        if (isBoolean(value)) {
            return (
        ...
        if (isString(value)) {
            ...
            const dropdownOptions: Record<string, string[]> = {
                ...
                region: ["eastus", "us-east-1", "us-west-2", "eu-west-1", "asia-south1", "southamerica-east1"],
                sku: ["P1v2", "B1", "S1"],
                ...
                ci_cd_tool: ["GitHub Actions", "GitLab CI", "CircleCI", "Bitbucket Pipelines"],
                secret_management: ["Azure Key Vault", "AWS Secrets Manager"],
                security_center_tier: ["Standard", "Free"]
            };
            if (dropdownOptions[label]) {
                return (
                    <label key={key} className="ic-toggle-label" style={{ marginBottom: 8 }}>
                        <span style={{ minWidth: 140 }}>{label.replace(/_/g, " ").replace(/\b\w/g, l => l.toUpperCase())}:</span>
                        <select
            ...
            return (
                <label key={key} className="ic-toggle-label" style={{ marginBottom: 8 }}>
                    ...
                    <input
                        ...
                        onChange={e =>
                            idx !== undefined
                                ? handleArrayChange(sectionPath, idx, label, Number(e.target.value))
            ...
            if (value.length > 0 && isString(value[0])) {
                return (
                    <div key={key} style={{ marginBottom: 10, width: "100%" }}>
                        ...
                        <div style={{ marginLeft: 16, borderLeft: "2px solid rgb(160, 158, 158)", paddingLeft: 12 }}>
                            <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%" }}>
                                {value.map((v: string, i: number) => (
                                    <input
                                        ...
                                        onChange={e => handleArrayStringChange(sectionPath, label, i, e.target.value)}
                                    />
                                ))}
                            </div>
                        </div>
                ...
                return (
                    <div key={key} style={{ marginBottom: 12, width: "100%" }}>
                        ...
                        {value.map((item: any, i: number) => (
                            <div key={i} style={{ marginLeft: 16, borderLeft: "2px solid rgb(160, 158, 158)", paddingLeft: 12, marginBottom: 10 }}>
